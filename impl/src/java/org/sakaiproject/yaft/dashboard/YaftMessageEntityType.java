...
public class YaftMessageEntityType extends YaftDashboardEntityType{
    ...
    public String getGroupTitle(int numberOfItems, String contextTitle, String labelKey) {
        ...
        return rl.getString("grouped_message_title");
    }

    public String getIdentifier() {
        return YaftMessageCreatedEventProcessor.IDENTIFIER;
    }

    public Map<String, Object> getValues(String entityReference, String localeCode) {

        ResourceLoader rl = new ResourceLoader("org.sakaiproject.yaft.impl.bundle.dashboard");
        String messageId = entityReference.substring(entityReference.lastIndexOf("/") + 1);
        Message message;
        try {
        ...
        infoItem.put(VALUE_INFO_LINK_URL, message.getUrl());
        ...
        values.put(VALUE_NEWS_TIME, df.format(new Date(message.getCreatedDate())));
        values.put(VALUE_MORE_INFO, infoList);
        values.put(VALUE_TITLE, message.getSubject());
        try {
            values.put(VALUE_USER_NAME, userDirectoryService.getUser(message.getCreatorId()).getDisplayName());
        } catch (UserNotDefinedException unde) {
        }
        values.put(DashboardEntityInfo.VALUE_ENTITY_TYPE, YaftMessageCreatedEventProcessor.IDENTIFIER);
        return values;
    }
}
