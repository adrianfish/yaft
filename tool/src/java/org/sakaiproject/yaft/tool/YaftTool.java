			...
			if (parts.length >= 1)
			{
				...
				else if ("forums".equals(part1))
				{
					...
					if (parts.length >= 2)
					{
						...
						if(parts.length == 2)
						{
							...
							response.sendRedirect("/sakai-yaft-tool/yaft.html?state=forum&forumId=" + forumId + "&siteId=" + siteId + "&placementId=" + placementId + "&language=" + languageCode);
							return;
						}
						else
						{
				...
				else if ("discussions".equals(part1))
				{
					if (parts.length >= 2)
					{
						...
						if(parts.length == 2)
						{
							...
							response.sendRedirect("/sakai-yaft-tool/yaft.html?state=full&discussionId=" + discussionId + "&siteId=" + siteId + "&placementId=" + placementId + "&forumId=" + forum.getId() + "&language=" + languageCode);
							return;
						}
						else
						{
				...
				else if ("messages".equals(part1))
				{
					if (parts.length >= 2)
					{
						...
						{
							response.sendRedirect("/sakai-yaft-tool/yaft.html?state=" + messageOp + "&discussionId=" + message.getDiscussionId() + "&siteId=" + siteId + "&placementId=" + placementId + "&forumId=" + forum.getId()  + "&language=" + languageCode + "#message-" + messageId);
							return;
						}
						else if("minimal".equals(messageOp))
						{
							response.sendRedirect("/sakai-yaft-tool/yaft.html?state=" + messageOp + "&discussionId=" + message.getDiscussionId() + "&messageId=" + messageId + "&siteId=" + siteId + "&placementId=" + placementId + "&forumId=" + forum.getId()  + "&language=" + languageCode);
							return;
						}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		else if (function.equals("createMessage"))
		{
			...
			if (messageBeingRepliedTo != null && messageBeingRepliedTo.length() > 0)
			{
				...
				yaftForumService.addOrUpdateMessage(forumId, message, true);
